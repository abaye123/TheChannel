...
import { YoutubePlayerComponent } from '../../chat/youtube-player/youtube-player.component';
...
import { ReportComponent } from '../../chat/message/report/report.component';
...
export class ThreadMessageComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit() {
        this.chatService.getEmojisList()
            ...
            .catch(() => this.toastrService.danger('', 'שגיאה בהגדרת אימוג\'ים'));
    ...
    editMessage(message: ChatMessage) {
        ...
        this.chatService.setEditThreadMessage(message);
    }
    ...
    setReact(id: number | undefined, react: string) {
        if (id && react)
            this.chatService.setReact(id, react).catch(() => this.toastrService.danger('', "יש להתחבר לחשבון בכדי להוסיף אימוג'י"));
    }
    ...
    replyInThread(message: ChatMessage) {
        const threadReply: ChatMessage = {
            replyTo: this.threadMessage?.id || message.id,
            isThread: true
        };
